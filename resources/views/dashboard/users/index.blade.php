                    ...
                    <div>
                        <nav class="flex" aria-label="Breadcrumb">
                            <ol class="inline-flex items-center space-x-1 sm:space-x-3 text-sm text-gray-600">
                                ...
                                <li class="before:content-['/'] before:mr-2 text-gray-400">Users</li>
                            </ol>
                        </nav>
                    </div>
                </div>
                    ...
                    <table id="table" class="min-w-full border text-left text-gray-700">
                        <thead class="bg-gray-100 text-gray-700 uppercase text-sm">
                            <tr>
                                ...
                                <th class="px-4 py-3 border">Nama Panggilan</th>
                                <th class="px-4 py-3 border">Jenis Kelamin</th>
                                <th class="px-4 py-3 border">Tipe Diabetes</th>
                        ...
                        <tbody>
                            @forelse ($users as $index => $user)
                                <tr @if($user->suspended) title="User dinonaktifkan" @endif
                                    class="@if($user->suspended) bg-red-200 hover:bg-red-100 @else hover:bg-gray-50 @endif">
                                    <td class="px-4 py-3 border">{{ $index + 1 }}</td>
                                    <td class="px-4 py-3 border"><code
                                            class="font-bold text-primary">{{ $user->username }}</code></td>
                                    <td class="px-4 py-3 border">{{ $user->fullname }}</td>
                                    <td class="px-4 py-3 border">{{ $user->callname }}</td>
                                    <td class="px-4 py-3 border">{{ ucfirst($user->gender ?? '-') }}</td>
                                    <td class="px-4 py-3 border">
                                        {{ $user->diabetes_type ? 'Tipe ' . $user->diabetes_type : '-'}}
                                    </td>
                                    <td class="px-4 py-3 border">{{ $user->email ?? '-'}}</td>
                                    <td class="px-4 py-3 border">{{ $user->no_hp ?? '-'}}</td>
                                    @if (Auth::user()->role == App\Models\User::ROLE_SUPERADMIN)
                                        <td class="px-4 py-3 border"><small
                                                class="font-medium rounded-md bg-blue-50 text-blue-800 ring-1 ring-inset ring-blue-700/20 text-xs py-1 px-3">{{ ucfirst($user->role) }}</small>
                                        </td>
                                    @endif
                                    <td class="px-4 py-3 border">
                                    ...
                                    <td class="px-4 py-3 border">
                                        <div class="flex flex-col lg:flex-row flex-wrap gap-2">
                                            ...
                                            @if (Auth::user()->role == App\Models\User::ROLE_SUPERADMIN)
                                                ...
                                                @if ($user->suspended)
                                                    <form title="Aktifkan user"
                                                        action="{{ route('dashboard.users.suspend', $user->id) }}" method="POST"
                                                        onsubmit="return confirm('Yakin ingin mengaktifkan user?')" class="inline">
                                                        @csrf
                                                        <button
                                                ...
                                                @else
                                                    <form title="Nonaktifkan user"
                                                        action="{{ route('dashboard.users.suspend', $user->id) }}" method="POST"
                                                        onsubmit="return confirm('Yakin ingin menonaktifkan user?')" class="inline">
                                                        @csrf
                                                        <button
                                                ...
                                                <form action="{{ route('dashboard.users.destroy', $user->id) }}" method="POST"
                                                    onsubmit="return confirmDelete('{{ $user->username }}')" class="inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit"
                                                        class="inline-flex items-center px-3 py-1 rounded bg-red-500 text-white hover:bg-red-600 text-sm"
                                                ...
                                                <script>
                                                    function confirmDelete(username) {
                                                        let input = prompt(`Ketik username untuk konfirmasi hapus: ${username}`);
                                                        if (input === null) {
                                                            return false;
                                                        }
                                                        ...
                                                        alert("Konfirmasi gagal! Username tidak sesuai.");
                                                        return false;
                                                    }
                                                </script>
