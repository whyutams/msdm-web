...
@section('content')
    <section class="py-24 bg-white">
        <div class="max-w-6xl mx-auto px-6 sm:px-8 lg:px-12">
            ...
            @endif
            <div class="flex justify-between mb-10 pb-6 border-b-2 border-slate-200">
                ...
                <div class="flex items-center justify-center max-w-xs">
                    <div class="items-center">
                        <a href="{{route('profile.form')}}"
                            class="bg-primary hidden lg:inline text-white px-8 py-3 rounded-xl text-base font-semibold hover:bg-blue-800 hover:shadow-lg transition-all duration-200 w-60 text-center">
                            Ubah Profil
                        </a>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6 text-lg text-gray-700">
                <div class="space-y-4">
                    <div>
                        ...
                        <p>{{ str_replace('_', ' ', Auth::user()->usia) }}</p>
                    </div>
                    <div>
                        <h6 class="font-bold text-xl mb-1">Pendidikan</h6>
                        <p>{{ format_label(Auth::user()->pendidikan) }}</p>
                    </div>
                    <div>
                        <h6 class="font-bold text-xl mb-1">Pekerjaan</h6>
                        <p>{{ format_label(Auth::user()->pekerjaan) }}</p>
                    </div>
                </div>
                <div class="space-y-4">
                    <div>
                        <h6 class="font-bold text-xl mb-1">Status Perkawinan</h6>
                        <p>{{ format_label(Auth::user()->status_perkawinan) }}</p>
                    </div>
                    <div>
                        <h6 class="font-bold text-xl mb-1">Lama DM</h6>
                        <p>{{ format_label(Auth::user()->lama_dm) }}</p>
                    </div>
                    <div>
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <p>{{ format_label(Auth::user()->pengobatan_dm) }}</p>
                    </div>
                    <div>
                        <h6 class="font-bold text-xl mb-1">Riwayat Keluarga</h6>
                        <p>{{ format_label(Auth::user()->riwayat_keluarga) }}</p>
                    </div>
                </div>
            ...
            <div class="mt-16 flex justify-between space-x-4">
                ...
                <a href="{{route('profile.form')}}"
                    class="bg-primary inline lg:hidden text-white px-8 py-3 rounded-xl text-base font-semibold hover:bg-blue-800 hover:shadow-lg transition-all duration-200 w-60 text-center">
                    Ubah Profil
                </a>
            </div>
        </div>
